...
# --- user name helpers (first/last registration + later change) ---
def set_user_fullname(uid: int, full_name: str) -> None:
    ...
    conn.execute(
        ...
        (uid, full_name, now_str()),
    )
    conn.commit()
    conn.close()

def get_user_fullname(uid: int) -> str:
    conn = db()
    r = conn.execute("SELECT full_name FROM users WHERE user_id=?", (uid,)).fetchone()
    conn.close()
    return (r["full_name"] if r and r["full_name"] else "")
...
def kb_user_home() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="✏️ Ism/familiyani o‘zgartirish", callback_data="u:chgname")],
    ])
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if not u:
        # ask name (first time)
        await message.answer(
            "👋 Salom! Ism va familiyangizni bitta xabarda kiriting (masalan: Ali Valiyev):"
        )
...
@router.callback_query(F.data == "u:chgname")
async def u_chgname(call: CallbackQuery, state: FSMContext):
    await state.clear()
    await state.update_data(name_mode="change")
    await call.message.answer("✏️ Ism va familiyangizni kiriting (masalan: Ali Valiyev):")
    await state.set_state(UState.reg_name)
    await call.answer()
...
async def cmd_name(message: Message, state: FSMContext):
    await state.clear()
    await state.update_data(name_mode="change")
    await message.answer("✏️ Ism va familiyangizni kiriting (masalan: Ali Valiyev):")
    await state.set_state(UState.reg_name)
...
async def reg_name(message: Message, state: FSMContext):
    ...
    if len(parts) < 2 or len(name) < 3:
        await message.answer("Iltimos, ism-familiyani to‘liq yozing (masalan: Ali Valiyev):")
        return

    # Normalize spacing
    full_name = " ".join(parts[:4])  # limit to reasonable length (4 words)

    data = await state.get_data()
    mode = data.get("name_mode") or "register"

    if mode == "change":
        set_user_fullname(message.from_user.id, full_name)
        await state.clear()
        await message.answer(
            f"✅ Ism/familiya yangilandi: <b>{escape_html(full_name)}</b>",
            parse_mode="HTML",
            reply_markup=kb_user_home(),
        )
    else:
        ensure_user(message.from_user.id, full_name)
        await state.clear()
